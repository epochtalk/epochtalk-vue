    ...
    <div class="recent-threads">
      ...
      <div class="threads-container" v-if="threads">
        ...
        <div class="threads-data threads-recent" v-for="thread in threads" :key="thread.id">
          ...
          <div class="title">
            <router-link class="bold thread-title" :title="decode(thread.title)" :to="{ name: 'Posts', params: { threadSlug: thread.slug }, query: (thread.post.position && thread.post.position > 1 ? { start: thread.post.position } : undefined) }" onclick="event.stopPropagation()">{{decode(thread.title)}}</router-link>
          </div>
          <div class="user">
            in
...
export default {
  setup() {
    onBeforeMount(() => {
      threadsApi.recent().then(recent => v.threads = recent).catch(() => {})
    })
    const v = reactive({
      threads: null
    })
    return { ...toRefs(v), humanDate, decode }
  }
}
</script>
