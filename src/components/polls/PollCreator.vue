<template>
  <div class="polls poll-form">
    <div class="section-poll-questions">
      ...
      <input type="text" id="question" v-model="pollCopy.question" maxlength="255">

      <!-- Poll Answers -->
      <label for="answer-1" class="poll-answers">Answers:</label>
      <div class="answer-row" v-for="(answer, index) in pollCopy.answers" :key="index">
        <div class="field-inline-action">
          <input type="text" :id="'answer-' + index" v-model="pollCopy.answers[index]" maxlength="255">
      ...
      <!-- Add Answer Button -->
      <div class="actions">
        <button @click="addPollAnswer()" :disabled="pollCopy.answers.length > 19" class="secondary small">Add an Answer</button>
      </div>
    </div>
    ...
    <div class="section-poll-options">
      ...
      <div class="checkbox-group">
        <input type="checkbox" id="pollVote" v-model="pollCopy.change_vote">
      ...
      <div class="poll-options-expiration">
        <label for="pollExpirationDate" class="fieldgroup-container">
          <span class="fieldgroup-label">Poll expires at -- {{humanDate(pollCopy.expiration) || '(No Expiration)'}}</span>
          <div class="fieldgroup">
            <input type="date" id="pollExpirationDate" v-model="pollCopy.expiration_date" @change="calcExpiration()">
            <input type="time" v-model="pollCopy.expiration_time" v-change="calcExpiration()">
          </div>
        </label>
      </div>
      ...
      <div class="poll-options-answers">
        ...
        <input type="number" id="pollMaxVote" min="1" :max="pollCopy.answers.length" v-model="pollCopy.max_answers">

        <div class="">
          <label>Show poll results: </label>
          <label for="displayAlways">
            <input type="radio" id="displayAlways" v-model="pollCopy.display_mode" value="always">
            Always shown
          </label>
          <label for="displayVoted">
            <input type="radio" id="displayVoted" v-model="pollCopy.display_mode" value="voted">
            After voting
          </label>
          <label for="displayExpired">
            <input type="radio" id="displayExpired" v-model="pollCopy.display_mode" value="expired" :disabled="!pollCopy.expiration">
            After expiration
          </label>
        </div>
      </div>
