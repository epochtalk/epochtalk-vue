  ...
  <modal :name="$options.name" :show="show" @close="close()" :focusInput="focusInput">
    ...
    <template v-slot:body>
        <form class="css-form">
          ...
          <input type="radio" class="pointer" name="ruleType" v-model="rule.type" :value="0" id="single" :disabled="requestSubmitted || remove" required />
          <label for="single">Single IP</label>

          <input type="radio" class="pointer" name="ruleType" v-model="rule.type" :value="1" id="range" :disabled="requestSubmitted || remove" required />
          <label for="range">IP Range</label>

          <input type="radio" class="pointer" name="ruleType" v-model="rule.type" :value="2" id="wildcard" :disabled="requestSubmitted || remove" required />
          ...
          <div v-if="rule.type === 1">
            ...
            <div class="col">
              <div>
                <input type="text" v-model="rule.start" placeholder="Start Ex: 203.0.113.0 or 2001:0db8:0000:0000:0000:0000:0000:0000" ng-pattern="ipRegex" :disabled="requestSubmitted || remove" required />
              </div>
              <div>
                <input type="text" v-model="rule.end" placeholder="End Ex: 203.0.113.255 or 2001:0db8:ffff:ffff:ffff:ffff:ffff:ffff" ng-pattern="ipRegex" :disabled="requestSubmitted || remove" required />
              </div>
            </div>
          </div>
          ...
          <div v-if="rule.type === 2">
            ...
            <div class="wildcard col">
              <input type="text" v-model="rule.blockOne" ng-pattern="blockRegex" placeholder="203" maxlength="3" :disabled="requestSubmitted || remove" required />
              <input type="text" v-model="rule.blockTwo" :disabled="requestSubmitted || remove" ng-pattern="blockWildcardRegex" placeholder="0" maxlength="3" required />
              <input type="text" v-model="rule.blockThree" :disabled="requestSubmitted || remove" ng-pattern="blockWildcardRegex" placeholder="113" maxlength="3" required />
              <input type="text" v-model="rule.blockFour" ng-pattern="blockWildcardRegex" placeholder="*" maxlength="3" :disabled="requestSubmitted || remove" required />
            </div>
          </div>

          <div class="col">
            <div>
              <button class="fill-row" v-if="add" @click.prevent="modifyRule(true)" :disabled="requestSubmitted" v-html="saveContinueBtnLabel"></button>
              <button class="fill-row negative" v-if="edit || remove" @click.prevent="close()" :disabled="requestSubmitted">Cancel</button>
            </div>
            <div>
              <button class="fill-row" @click.prevent="modifyRule()" :disabled="requestSubmitted" v-html="saveRuleBtnLabel"></button>
            </div>
          </div>
