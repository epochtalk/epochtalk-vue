<template>
<div class="user-profile-posts">
  <div class="table-actions">
    <button ng-click="ProfilePostsCtrl.toggleThreads(true)" ng-class="{'active': ProfilePostsCtrl.threads}">
      <strong>Threads&nbsp;</strong> <span class="username">by {{ProfilePostsCtrl.user.username}}</span>
    </button>

    <button ng-click="ProfilePostsCtrl.toggleThreads(false)" ng-class="{'active': !ProfilePostsCtrl.threads}">
      <strong>Posts&nbsp;</strong> <span class="username">by {{ProfilePostsCtrl.user.username}}</span>
    </button>
  </div>

  <!-- User's Paged Posts -->
  <div ng-if="!ProfilePostsCtrl.threads">
    <div class="">
      <div class="pagination-wrap" ng-if="ProfilePostsCtrl.pageCount > 0">
        <pagination page-count="ProfilePostsCtrl.pageCount" page="ProfilePostsCtrl.page" query-params="ProfilePostsCtrl.queryParams"></pagination>
      </div>
    </div>

    <div class="no-table-contents" ng-if="ProfilePostsCtrl.pageCount === 0">
      <h4>This user has no posts to display</h4>
    </div>

    <div class="" ng-if="ProfilePostsCtrl.pageCount > 0">
      <table class="profile-posts-table" width="100%">
        <thead>
          <th class="thread">Thread</th>
          <th class="timestamp pointer" ng-click="ProfilePostsCtrl.setDesc()"><span ng-class="ProfilePostsCtrl.getDesc()"></span>&nbsp;Post Date</th>
        </thead>

        <tbody>
          <tr ng-repeat="post in ProfilePostsCtrl.usersPosts track by post.id">
            <td data-balloon="{{post.thread_title}}" data-balloon-pos="top">
              <div class="truncate-title">
                <span ng-if="post.hidden && !post._deleted"><i class="fa fa-eye-slash"></i></span>
                <a ui-sref="posts.data({ slug: post.thread_slug, start: post.position, '#': post.id })" ng-bind-html="post.thread_title" ng-if="!post._deleted" class="thread-title"></a>
                <span ng-if="post._deleted">Deleted</span>
              </div>
              <div class="post" ng-click="post.open = !post.open">
                <div class="post-body" ng-class="{ closed: !post.open }" post-processing="post.body_html" style-fix="true"></div>
              </div>
            </td>
            <td ng-bind="post.created_at | date:'MM/dd/yy @ h:mma'" class="timestamp"></td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>


  <div ng-if="ProfilePostsCtrl.threads">
    <!-- User's Paged Threads -->

      <div class="pagination-wrap" ng-if="ProfilePostsCtrl.usersPosts.length > 0">
        <div class="pagination-simple">
          <a ng-show="ProfilePostsCtrl.prev" ui-sref=".({ threads: ProfilePostsCtrl.threads, page: ProfilePostsCtrl.prev, limit: ProfilePostsCtrl.limit, desc: ProfilePostsCtrl.desc })">
            &#10094; Previous
          </a>
          <span ng-hide="ProfilePostsCtrl.prev">&#10094; Previous</span>
          &nbsp;&nbsp;&nbsp;
          <a ng-show="ProfilePostsCtrl.next" ui-sref=".({ threads: ProfilePostsCtrl.threads, page: ProfilePostsCtrl.next, limit: ProfilePostsCtrl.limit, desc: ProfilePostsCtrl.desc })">
             Next &#10095;
          </a>
          <span ng-hide="ProfilePostsCtrl.next">Next &#10095;</span>
        </div>
      </div>

    <div class="no-table-contents" ng-if="ProfilePostsCtrl.usersPosts.length < 1">
      <h4>This user has no threads to display</h4>
    </div>

    <div class="" ng-if="ProfilePostsCtrl.usersPosts.length > 0">
      <table class="profile-posts-table" width="100%">
        <thead>
          <th class="thread">Thread</th>
          <th class="timestamp pointer" ng-click="ProfilePostsCtrl.setDesc()"><span ng-class="ProfilePostsCtrl.getDesc()"></span>&nbsp;Post Date</th>
        </thead>

        <tbody>
          <tr ng-repeat="post in ProfilePostsCtrl.usersPosts track by post.id">
            <td data-balloon="{{post.thread_title}}" data-balloon-pos="top">
              <div class="truncate-title">
                <span ng-if="post.hidden && !post._deleted"><i class="fa fa-eye-slash"></i></span>
                <a ui-sref="posts.data({ slug: post.thread_slug, start: post.position, '#': post.id })" ng-bind-html="post.thread_title" ng-if="!post._deleted" class="thread-title"></a>
                <span ng-if="post._deleted">Deleted</span>
              </div>
              <div class="post" ng-click="post.open = !post.open">
                <div class="post-body" ng-class="{ closed: !post.open }" post-processing="post.body_html" style-fix="true"></div>
              </div>
            </td>
            <!--
            <td style="display: none" ng-click="post.open = !post.open">
              <div class="post-body" ng-class="{ closed: !post.open }" post-processing="post.body_html" style-fix="true"></div>
            </td> -->
            <td ng-bind="post.created_at | date:'MM/dd/yy @ h:mma'" class="timestamp"></td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
</div>

</template>

<script>
import { reactive, toRefs } from 'vue'

export default {
  name: 'Profile',
  setup() {
    const v = reactive({
      test: false
    })
    return { ...toRefs(v) }
  }
}
</script>

<style lang="scss">

</style>
